...
const regex = /<div key=\{group\.id\} id=\{group\.id\} className="group-section[\s\S]*?<div className="overflow-x-auto">/m;

const correctCode = `<div key={group.id} id={group.id} className="group-section scroll-mt-24 rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden">
                <div className="px-4 py-3 bg-muted/40 flex justify-between items-center break-inside-avoid">
                  ...
                  >
                    {group.terms.length > 0 && group.terms.every((t: any) => selectedMap[t.id]) ? (
                      <><CheckSquare className="w-4 h-4 text-primary" /> تحديد هذه المجموعة</>
                    ) : (
...
const fixed = content.replace(regex, correctCode);
